...
# Regrets CFR par phase
REGRETS_CFR_FLOP  = {}
REGRETS_CFR_TURN  = {}
REGRETS_CFR_RIVER = {}

# Compteurs d'appels par phase
CALL_COUNT_FLOP  = 0
CALL_COUNT_TURN  = 0
CALL_COUNT_RIVER = 0

# Taux d'apprentissage par phase
ALPHA_FLOP  = 0.1
ALPHA_TURN  = 0.1
ALPHA_RIVER = 0.1
...
# Paramètre Monte-Carlo pour bluff
MONTE_CARLO_SAMPLES = 20

# Poids de la fusion CFR / Q (ex. 50% CFR, 50% Q)
CFR_WEIGHT = 0.5
...
def get_flop_action(
    player_name,
    best_hand,      # Évaluation numérique (1 à 9)
    highest_hand,   # Évaluation numérique (1 à 9)
    min_amount,
    max_amount,
    street,         # "flop"
    pot,
    ...
    global CALL_COUNT_FLOP, Q_TABLE_FLOP, REGRETS_CFR_FLOP
    CALL_COUNT_FLOP += 1
    ...
    logger.info(f"[FLOP] opp_aggr_ratio={opp_aggr_ratio:.2f}, opp_sd_ratio={opp_sd_ratio:.2f}")
    ...
    weighted_avg = (w_basic * sb) + (w_pro * sp) + 0.3 * connectivity_bonus
    if weighted_avg >= 8:
        final_class = "monster"
    elif weighted_avg >= 7:
    ...
    logger.info(f"[FLOP] basic={c_basic}, pro={c_pro}, connectivity={connectivity_bonus:.2f} => final_class={final_class}")
    ...
    q_vals = {a: Q_TABLE_FLOP.get((final_class, a), 0.0) for a in poss_actions}
    eps = dynamic_epsilon(CALL_COUNT_FLOP, opp_aggr_ratio)
    temp = dynamic_temperature(CALL_COUNT_FLOP)
    q_policy = softmax(q_vals, temp)
    cfr_policy = cfr_policy_from_regrets(REGRETS_CFR_FLOP, final_class, poss_actions)
    combined_policy = {}
    ...
    cfr_update_regrets(REGRETS_CFR_FLOP, final_class, action, rew, poss_actions)

    new_q_val = Q_TABLE_FLOP[key]
    reg_val = REGRETS_CFR_FLOP.get(key, 0.0)
    logger.info(f"[FLOP] final_class={final_class}, action={action}, amount={amount:.2f}, reward={rew:.2f}")
    logger.info(f"[FLOP] Q_TABLE[{key}]={new_q_val:.2f}, regret={reg_val:.2f}")
...
def get_turn_action(
    ...
    global CALL_COUNT_TURN, Q_TABLE_TURN, REGRETS_CFR_TURN
    CALL_COUNT_TURN += 1
    ...
    final_class = classify_turn(best_hand, highest_hand)
    ...
    poss_actions = ["fold", "call", "raise"]
    q_vals = {a: Q_TABLE_TURN.get((final_class, a), 0.0) for a in poss_actions}
    eps = dynamic_epsilon(CALL_COUNT_TURN, opp_aggr_ratio)
    temp = dynamic_temperature(CALL_COUNT_TURN)
    q_policy = softmax(q_vals, temp)
    cfr_policy = cfr_policy_from_regrets(REGRETS_CFR_TURN, final_class, poss_actions)
    combined_policy = {}
    for a in poss_actions:
        combined_policy[a] = CFR_WEIGHT * cfr_policy.get(a, 0.0) + (1 - CFR_WEIGHT) * q_policy.get(a, 0.0)
    total_comb = sum(combined_policy.values())
    ...
    for a, p in combined_policy.items():
        ...
        if r_draw < cumul:
            ...
            break
    if not chosen_act:
        chosen_act = max(combined_policy, key=combined_policy.get)
    ...
    cfr_update_regrets(REGRETS_CFR_TURN, final_class, action, rew, poss_actions)

    new_q = Q_TABLE_TURN[key]
    new_reg = REGRETS_CFR_TURN.get(key, 0.0)
    print(f"[TURN] final_class={final_class}, action={action}, amount={amount:.2f}, reward={rew:.2f}")
    print(f"[TURN] Q_TABLE[{key}]={new_q:.2f}, regret={new_reg:.2f}")
    return action, amount
...
def get_river_action(
    ...
    global CALL_COUNT_RIVER, Q_TABLE_RIVER, REGRETS_CFR_RIVER
    CALL_COUNT_RIVER += 1
    ...
    final_class = classify_river(best_hand, highest_hand)
    ...
    poss_actions = ["fold", "call", "raise"]
    q_vals = {a: Q_TABLE_RIVER.get((final_class, a), 0.0) for a in poss_actions}
    eps = dynamic_epsilon(CALL_COUNT_RIVER, opp_aggr_ratio)
    temp = dynamic_temperature(CALL_COUNT_RIVER)
    q_policy = softmax(q_vals, temp)
    cfr_policy = cfr_policy_from_regrets(REGRETS_CFR_RIVER, final_class, poss_actions)
    combined_pol = {}
    for a in poss_actions:
        combined_pol[a] = CFR_WEIGHT * cfr_policy.get(a, 0.0) + (1 - CFR_WEIGHT) * q_policy.get(a, 0.0)
    total_comb = sum(combined_pol.values())
    if total_comb <= 0:
        combined_pol = {a: 1.0 / len(poss_actions) for a in poss_actions}
    else:
        for a in combined_pol:
            combined_pol[a] /= total_comb

    r_draw = random()
    cumul = 0.0
    chosen_act = None
    for a, p in combined_pol.items():
        cumul += p
        if r_draw < cumul:
            chosen_act = a
            break
    if not chosen_act:
        chosen_act = max(combined_pol, key=combined_pol.get)
    ...
    cfr_update_regrets(REGRETS_CFR_RIVER, final_class, action, rew, poss_actions)

    new_q = Q_TABLE_RIVER[key]
    reg_val = REGRETS_CFR_RIVER.get(key, 0.0)
    print(f"[RIVER] final_class={final_class}, action={action}, amount={amount:.2f}, reward={rew:.2f}")
    print(f"[RIVER] Q_TABLE[{key}]={new_q:.2f}, regret={reg_val:.2f}")
    return action, amount

# =====================================================================
# Pour compatibilité avec le loader, si nécessaire, on crée un alias
# pour get_card_action qui ici pointe vers la stratégie FLOP.
# =====================================================================
get_card_action = get_flop_action
