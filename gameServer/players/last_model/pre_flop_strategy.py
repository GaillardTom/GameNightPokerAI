...
# ------------------------ Paramètres Globaux ------------------------
Q_TABLE_PREFLOP = {}         # Q‑table pour le préflop
ALPHA_PREFLOP = 0.1          # Taux d'apprentissage pour le préflop
...
TEMPERATURE_DECAY = 0.005    # Décroissance de la température par appel
...
def dynamic_epsilon(call_count, opp_aggr_ratio):
    """Retourne un epsilon dynamique décroissant avec l'expérience, augmenté si l'adversaire est agressif."""
...
def shape_reward(base_reward, opp_aggr_ratio, opp_showdown_ratio, action, final_class, pot, min_amount):
    """
    Ajuste la récompense de base en tenant compte de l'agressivité adverse (préflop et showdown)
    et du potentiel de gain. On renforce les bonus pour les mains fortes face à des adversaires agressifs.
    """
    bonus = 1.0
...
# ------------------------ STRATÉGIE PREFLOP ------------------------
# (Ici, la stratégie préflop est laissée minimale pour se concentrer sur FLOP. Vous pouvez l'adapter
# de manière similaire si besoin.)
...
def get_pre_flop_action(
    ...
    # Pour cet exemple, on retourne simplement "call".
    logger.info("[PREFLOP] Using baseline preflop strategy: call")
...
def get_card_action(
    ...
    Intègre :
      1. Évaluation de la main via trois approches (ensembles, Chen, Sklansky)
         avec un bonus de connectivité.
      ...
      4. Renforcement par Q‑learning avec epsilon dynamique, softmax et reward shaping,
         anticipant un adversaire agressif au showdown.
    """
    ...
    # Bonus de connectivité
    # Récupère les hole cards depuis l'historique s'ils existent, sinon utilise une valeur par défaut (vide)
    ...
    # Mise à jour de la Q-table avec reward shaping
