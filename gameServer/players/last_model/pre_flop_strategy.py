...
MONTE_CARLO_SAMPLES = 20     # Nombre d'échantillons pour bluff Monte-Carlo

# Exemple de stats adverses (extraction possible depuis l'historique)
OPPONENT_STATS = {
    "fold_to_raise_ratio": 0.0,
...
def softmax(q_dict, temperature):
    ...
    exp_vals = {act: math.exp((v - max_q)/temperature) for act, v in q_dict.items()}
...
# ===================== CFR: Policy Extraction et Update =====================
...
def shape_reward(base_reward, opp_aggr_ratio, opp_sd_ratio, action, final_class, pot, min_amount, logger):
    bonus = 1.0
    ...
    if opp_sd_ratio > 0.6 and final_class not in {"monster", "ultra_premium", "premium"}:
        bonus *= 0.8
...
def get_pre_flop_action(
    ...
    # Exemple de classification simplifiée pour préflop
    uprem = {"AA", "KK"}
    prem = {"QQ", "JJ", "AKs"}
    ...
    # Arbre de décision de base préflop
    action = "fold"
    amount = 0.0
    ...
    rew = shape_reward(base_r, opp_aggr_ratio, opp_sd_ratio, action, final_class, pot, min_amount, logger)
    key = (final_class, action)
    old_q = Q_TABLE_PREFLOP.get(key, 0.0)
    Q_TABLE_PREFLOP[key] = (1 - ALPHA_PREFLOP) * old_q + ALPHA_PREFLOP * rew

    cfr_update_regrets(REGRETS_CFR_PREFLOP, final_class, action, rew, poss_actions)

    new_q_val = Q_TABLE_PREFLOP[key]
    reg_val = REGRETS_CFR_PREFLOP.get(key, 0.0)
    logger.info(f"[PREFLOP] final_class={final_class}, action={action}, amount={amount:.2f}, reward={rew:.2f}")
    logger.info(f"[PREFLOP] Q_TABLE[{key}]={new_q_val:.2f}, regret={reg_val:.2f}")

    return action, amount

# =====================================================================
# Pour compatibilité avec le loader, on peut définir un alias si nécessaire.
# Par exemple, si le loader cherche "get_card_action", on l'assigne à get_pre_flop_action.
# =====================================================================
get_card_action = get_pre_flop_action
