...
def get_card_action(
    ...
    def sum_raises(action_histories):
        ...
        return sum(
            action["action"] in ["RAISE", "CALL"] for actions in action_histories.values() for action in actions if action["name"] == player_name
        )    
    ...
    opponent_aggressive = any(is_opponent_aggressive() for actions in action_histories.values())
